...
def filter_dataframe(df, algn_file):
    '''Takes a data and headers lists from parse_hyphy and filter the positions corresponding to the Human sequence. Returns a pandas dataframe'''
...
def assign_selective_pressure(df):
    '''Takes a datafame with hyphy results and assign a column displaying the type of selective pressure in each residue.'''
...
def main():
    ...
    parser.add_argument('hyphy', help='File with hyphy results in .json format')
    parser.add_argument('Alignment', help='Sequence alignment used to filter codons corresponding to human sequence')
